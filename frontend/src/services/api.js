...
export async function signUp(payload) {
  ...
  if (data.token) {
    localStorage.setItem('yarntales-token', data.token)
  }
...
export async function updateCartItem(
  ...
  return request(
    `/api/cart/${encodeURIComponent(userId)}/${encodeURIComponent(productId)}`,
    {
      method: 'PUT',
...
export async function getWishlist(userId) {
  const data = await request(
    `/api/wishlist/${encodeURIComponent(userId)}`
  )
...
export async function removeWishlistItem(
  ...
  return request(
    `/api/wishlist/${encodeURIComponent(userId)}/${encodeURIComponent(productId)}`,
    {
      method: 'DELETE',
    }
  )
...
export async function getCustomizations(userId) {
  const data = await request(
    `/api/customizations/user/${encodeURIComponent(userId)}`
  )
...
export async function getGiftRequests(userId) {
  const data = await request(
    `/api/gift-mode/user/${encodeURIComponent(userId)}`
  )
...
// kept on the device (per signed-in user) using localStorage.
// ------------------------------------------------------------------

const addressesKey = (userId) =>
  `yarntales-addresses-${userId}`
...
export function getSavedAddresses(userId) {
  ...
  try {
    const list = JSON.parse(
      localStorage.getItem(addressesKey(userId))
    )
...
export function saveAddress(userId, address) {
  ...
  const entry = {
    ...
    isDefault:
      Boolean(address.isDefault) || isFirst,
  }
...
export function deleteAddress(
  ...
  const list = getSavedAddresses(userId).filter(
    (item) => item.id !== addressId
  )

  // If the deleted address was the default, promote
  // the next one so there is always a default address
  // when at least one address remains.
  if (
    list.length &&
    !list.some((item) => item.isDefault)
  ) {
    list[0] = {
      ...list[0],
      isDefault: true,
...
export function setDefaultAddress(
  ...
  const list = getSavedAddresses(userId).map(
    (item) => ({
      ...item,
